...
class ReportService:
    ...
    def _populate_full_sheet(ws, df: pd.DataFrame, metadata: Dict[str, Any], settings: Dict[str, Any], direction: str):
        ...
        max_template_blocks = 31 # We natively expanded it to 31 days
        for redundant_idx in range(len(days) + 1, max_template_blocks + 2):
            r_start = 58 + (redundant_idx - 1) * block_height
            # Add early exit if we've hidden everything available
    ...
    def _inject_summary_block(ws, df: pd.DataFrame, start_row: int, vmax: float, periods: List[str], metadata: Dict[str, Any], direction: str):
        ...
        if is_synthese:
            # J=% TMJ=L TMH=N Moy=P R=V15 T=V50 V=V85 X=Ecart Z=Infrac AA/AB=%Infrac P1=AC
        ...
        for i, v_class in enumerate(['Total', 'VL', 'PL']):
            ...
            # Ensure filtering respects 'Total' direction
            sub = df.copy()
            ...
            ReportService._safe_write(ws, target_row, col_row := col_map['inf'], int(metrics['infractions']))
            
            # 2. Synthesis Special (%, StdDev, %Inf)
            if is_synthese:
